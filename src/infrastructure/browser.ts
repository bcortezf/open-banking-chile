...
export interface BrowserOptions {
  ...
  /** Connect to an already-running Chrome (http://host:9222). */
  browserURL?: string;
...
export interface BrowserSession {
  ...
  page: Page;
  debugLog: string[];
  /** True si el browser ya existía (puppeteer.connect); no hay que cerrarlo. */
...
export async function launchBrowser(
  ...
  const connectUrl = (browserURL || process.env.BROWSER_URL || "").trim();
  const connectWs = (browserWSEndpoint || process.env.BROWSER_WS_ENDPOINT || "").trim();
  let browser: Browser;
  ...
  if (connectUrl || connectWs) {
    ...
    const useHeadful = Boolean(forceHeadful || headful);
  ...
  // En Chrome externo no forzar viewport CDP (puede delatar automatización).
  if (!externalBrowser) {
  ...
  return { browser, page, debugLog, externalBrowser, screenshot: doSave };
}
